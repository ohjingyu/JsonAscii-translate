...
**JSON · ASCII · 진수 · URL · QR · Unicode · Base64 · 인코딩 · 체크섬 · 네트워크 · 전자회로 — 개발자용 텍스트/데이터 변환기**
...
| **체크섬 / CRC** | 텍스트 또는 16진수 바이트를 입력하면 XOR-8, SUM-8, SUM-16, CRC-8, CRC-16(CCITT-FALSE / MODBUS / XMODEM), CRC-32까지 한 번에 계산합니다. 시리얼·통신 프로토콜 디버깅에 바로 쓸 수 있습니다. |
| **MAC / IP 변환** | IPv4를 10진수·16진수·2진수로 자동 변환하고, CIDR을 입력하면 네트워크/브로드캐스트 주소·서브넷 마스크·사용 가능 호스트 범위를 계산합니다. MAC 주소는 콜론·하이픈·Cisco 점 표기·구분자 없음·10진수·16진수 형식을 서로 변환합니다. |
| **전압 분배기** | Vin, R1, R2를 입력하면 Vout·전류·각 저항의 소비전력을 즉시 계산합니다. 반대로 목표 Vout과 저항 하나만 알 때 나머지 저항값을 역산하고, 가장 가까운 E12 표준 저항값도 함께 추천합니다. |
